...
| 8 | RTM 空白 | sda-tester agent 补充测试用例，确保需求与测试一一对应 |
...
- **注意**：RTM 是功能开发过程中按需创建的，不属于第三章初始化的 38 个新文件。每个功能/特性开发时独立创建，位于 `.claude/specs/<feature>/rtm.md`

**责任归属**：
- **创建**：sda-tester agent（或开发者），从需求文档提取 AC/ST/PM/BF 清单
- **维护**：开发过程中新增需求时同步更新 RTM
- **审计**：sda-coverage-auditor agent 独立验证 RTM 覆盖率真实性
...
**方法一：Markdown 表格格式检查**（适用于 AI 生成的标准格式 RTM）：
```bash
# 检查 RTM 是否有空白项（假设 RTM 文件在 .claude/specs/*/rtm.md）
# 空白项特征：表格最后一列为空（只有空格或无内容）
grep -E "^\| [A-Z]+-[0-9]+ \| [^|]+ \|  *\|" .claude/specs/*/rtm.md
# 如有输出则表示存在空白项，需要补充测试用例
```

**方法二：内容语义检查**（更鲁棒，不依赖精确的 Markdown 表格格式）：
```bash
# 提取所有需求 ID（AC/ST/PM/BF-xxx），检查每个 ID 是否有对应的测试用例路径
# 1. 提取所有需求 ID
grep -oE '[A-Z]+-[0-9]+' .claude/specs/*/rtm.md | sort -u > /tmp/rtm_ids.txt
# 2. 提取有测试用例关联的需求 ID（假设测试用例路径包含 tests/ 或 .test. 等特征）
grep -E '(tests/|\.test\.|\.spec\.)' .claude/specs/*/rtm.md | grep -oE '[A-Z]+-[0-9]+' | sort -u > /tmp/rtm_covered.txt
# 3. 对比找出未覆盖的需求
comm -23 /tmp/rtm_ids.txt /tmp/rtm_covered.txt
# 如有输出则表示存在未覆盖的需求
```

**方法三：AI 辅助检查**（推荐，最可靠）：
```
请检查 .claude/specs/*/rtm.md 中的需求追踪矩阵：
1. 列出所有需求 ID（AC/ST/PM/BF-xxx）
2. 检查每个需求是否有对应的测试用例路径
3. 检查状态列是否全部为 ✅（排除 ⚠️ 缺失、❌ 阻塞等）
4. 输出空白项列表
```
...
11. 从需求文档提取四类编号化清单：
    ...
    - 权限矩阵（PM）：角色 × 操作 × 预期结果
    - 业务流程（BF）：端到端完整流程
12. 构建 RTM（需求追踪矩阵），标记所有 GAP。**RTM 详细说明和模板见第五章 5.5 节**。RTM 是功能开发过程中按需创建的（`.claude/specs/<feature>/rtm.md`），不属于第三章初始化的 38 个新文件
13. 补写测试直到 RTM 零空白
...
- E2E 测试：`{{E2E_TEST_CMD}}`（如无 E2E 测试则删除此行）
- 覆盖率：`{{COVERAGE_CMD}}`
...
| 8 | RTM 空白 | 补充测试用例，确保需求与测试一一对应 | sda-tester agent |
...
| 1.2 | 2026-04-19 | 新增智能体质量门禁体系（nil 集合兜底、防御性数据消费、测试三层验证、RTM） |
...
| 1.5 | 2026-04-25 | 评审修订：统一文件计数（37个）、统一 Plan 触发条件（3个及以上文件）、统一写文件行数（200行）；明确"测试用例文档先行"而非 TDD；补充 SDA 阶段描述（Spec/实现）；完善 sdc-dev 自动编排调度 prompt；增加 E2E 框架配置检查；完善 RTM 验证方法；补充 PowerShell 脚本模板；删除 SDA model 设置；补充占位符 grep 排除说明；质量门禁归入 domain/；删除 MCP 相关配置 |
...
> **测试用例文档先行**：基于 requirements.md 中的 AC 和 RTM 四类清单，补充 test-cases.md 测试用例文档。注意：这是编写测试用例文档（描述预期行为），而非编写测试代码。
...
> 测试用例完成后，更新 `.claude/specs/<feature>/rtm.md` 中的测试用例列。
> 详见手册 5.5 节「RTM 需求追踪矩阵」。
```
