...
**需要复制的文件清单（31 个，原 34 个中 3 个 steer/foundation 文件已合并到 CLAUDE.md）**：
...
| 4 | rules/git.md | .claude/rules/git.md（已合并到 governance.md） |
| 5 | rules/collaboration.md | .claude/rules/collaboration.md（已合并到 governance.md） |
| 6 | ~~steer/foundation/tech.md~~ | ~~已合并到 CLAUDE.md~~ |
| 7 | ~~steer/foundation/structure.md~~ | ~~已合并到 CLAUDE.md~~ |
| 8 | ~~steer/foundation/never.md~~ | ~~已合并到 CLAUDE.md~~ |
| 9 | rules/quality-gate.md | .claude/rules/governance.md（已合并到 governance.md） |
| 10 | steer/domain/api.md | .claude/steer/domain/api.md |
| 11 | steer/domain/testing.md | .claude/steer/domain/testing.md |
| 12 | steer/domain/security.md | .claude/steer/domain/security.md |
| 13 | steer/domain/frontend.md | .claude/steer/domain/frontend.md |
| 14 | skills/sds-plan.md | .claude/skills/sds-plan.md |
| 15 | skills/sds-codereview.md | .claude/skills/sds-codereview.md |
| 16 | skills/sds-buildfix.md | .claude/skills/sds-buildfix.md |
| 17 | skills/sds-spec.md | .claude/skills/sds-spec.md |
| 18 | skills/sds-dev.md | .claude/skills/sds-dev.md |
| 19 | agents/sda-architect.md | .claude/agents/sda-architect.md |
| 20 | agents/sda-doc-reviewer.md | .claude/agents/sda-doc-reviewer.md |
| 21 | agents/sda-db-implementer.md | .claude/agents/sda-db-implementer.md |
| 22 | agents/sda-backend.md | .claude/agents/sda-backend.md |
| 23 | agents/sda-frontend.md | .claude/agents/sda-frontend.md |
| 24 | agents/sda-tester.md | .claude/agents/sda-tester.md |
| 25 | agents/sda-code-reviewer.md | .claude/agents/sda-code-reviewer.md |
| 26 | agents/sda-build-error-resolver.md | .claude/agents/sda-build-error-resolver.md |
| 27 | skills/troubleshooting.md | .claude/knowledge/troubleshooting.md |
| 28 | settings.json | .claude/settings.json |
| 29 | hooks/pre-bash-check.ps1 | .claude/hooks/pre-bash-check.ps1 |
| 30 | hooks/post-edit-check.ps1 | .claude/hooks/post-edit-check.ps1 |
| 31 | templates/spec/requirements.md | .claude/templates/spec/requirements.md |
| 32 | templates/spec/design.md | .claude/templates/spec/design.md |
| 33 | templates/spec/test-cases.md | .claude/templates/spec/test-cases.md |
| 34 | templates/spec/tasks.md | .claude/templates/spec/tasks.md |
...
```bash
# 先创建目录结构
mkdir -p .claude/rules .claude/commands .claude/agents .claude/skills .claude/knowledge \
  .claude/steer/domain \
...
# git.md、collaboration.md、quality-gate.md 已合并为 governance.md
cp claudeConfig/rules/governance.md .claude/rules/governance.md
...
cp claudeConfig/skills/sds-plan.md .claude/skills/sds-plan.md
cp claudeConfig/skills/sds-codereview.md .claude/skills/sds-codereview.md
cp claudeConfig/skills/sds-buildfix.md .claude/skills/sds-buildfix.md
cp claudeConfig/skills/sds-spec.md .claude/skills/sds-spec.md
cp claudeConfig/skills/sds-dev.md .claude/skills/sds-dev.md
...
cp claudeConfig/skills/troubleshooting.md .claude/knowledge/troubleshooting.md
...
{{CORE_DIR}} → {CORE_DIR}（禁止修改的目录，填入 CLAUDE.md）
...
| 2 | ~~.claude/steer/foundation/tech.md~~ | ~~已合并到 CLAUDE.md~~ |
| 3 | ~~.claude/steer/foundation/structure.md~~ | ~~已合并到 CLAUDE.md~~ |
| 4 | ~~.claude/steer/foundation/never.md~~ | ~~已合并到 CLAUDE.md~~ |
| 5 | .claude/steer/domain/api.md | 无 `{{...}}` 占位符，需手动编辑 fileMatchPattern |
| 6 | .claude/steer/domain/testing.md | 无 `{{...}}` 占位符，直接复制使用 |
| 7 | .claude/rules/governance.md | 无 `{{...}}` 占位符，直接复制使用（原 git+collaboration+quality-gate 合并） |
| 8 | .claude/steer/domain/security.md | 无 `{{...}}` 占位符，直接复制使用 |
| 9 | .claude/steer/domain/frontend.md | 无 `{{...}}` 占位符，直接复制使用 |
| 10 | .claude/rules/testing.md | 含 `{{TEST_CMD}}`、`{{CURRENT_COVERAGE}}`、`{{COVERAGE_CMD}}`、`{{LINT_CMD}}` 占位符，需替换 |
| 11 | .claude/rules/frontend.md | 含 `{{PLACEHOLDER}}` 占位符（文档性引用，非需替换），但会导致占位符检测误报（见 5.2 说明） |
| 12 | .claude/settings.json | 用扫描结果替换 `{{PACKAGE_MANAGER}}` |
| 13 | .claude/hooks/pre-bash-check.ps1 | 无 `{{...}}` 占位符，需手动编辑 PowerShell 变量（`$PACKAGE_MANAGER`） |
| 14 | .claude/hooks/post-edit-check.ps1 | 无 `{{...}}` 占位符，需手动编辑 PowerShell 变量（`$FILE_EXTENSIONS`、格式化提醒文本） |

> **注意**：rules/security.md、rules/governance.md（含原 git+collaboration+quality-gate）已包含实际规范内容，仅需确认分档标记，不包含 `{{...}}` 占位符。rules/testing.md 和 rules/frontend.md 含有少量占位符，需在替换步骤中处理。commands/ 和 agents/ 目录下的文件也不包含占位符，直接复制使用。
...
>
> **命名约定说明**：以下占位符中存在两套技术栈命名体系，用途不同，不可混淆：
> - `{{TECH_STACK}}`（CLAUDE.md）→ 综合描述，如"SpringBoot + Vue3"
> - `{{FRONTEND_FRAMEWORK}}` / `{{BACKEND_FRAMEWORK}}`（CLAUDE.md）→ 具体框架 + 版本，如"Vue3 + Element Plus" / "SpringBoot 2.7"
> 同理，`{{DB_TYPE}}`（CLAUDE.md）是综合描述，`{{DATABASE}}`（CLAUDE.md）是具体名称+版本。

| 占位符 | 含义 | 示例值 | 主要使用文件 |
|--------|------|--------|-------------|
| `{{PROJECT_NAME}}` | 项目名称 | ruoyi-admin | CLAUDE.md |
| `{{TECH_STACK}}` | 前端/后端框架+语言 | SpringBoot + RuoYi + JDK8 + Vue3 | CLAUDE.md |
| `{{DB_TYPE}}` | 数据库类型及版本（综合描述） | MySQL 8.0 / 瀚高 / PostgreSQL 13 | CLAUDE.md |
| `{{MIDDLEWARE}}` | 中间件名称及用途 | Redis（缓存/会话）/ ActiveMQ（异步通知） | CLAUDE.md |
| `{{PACKAGE_MANAGER}}` | 项目包管理器 | Maven / pnpm / pip / yarn | CLAUDE.md、settings.json、hooks |
| `{{BUILD_CMD}}` | 构建命令 | mvn package / pnpm build | CLAUDE.md |
| `{{TEST_CMD}}` | 单元测试命令 | mvn test / pnpm test | CLAUDE.md、rules/testing.md |
| `{{E2E_TEST_CMD}}` | E2E测试命令（如无则标注"无"） | pnpm test:e2e | CLAUDE.md |
| `{{FORMAT_CMD}}` | 代码格式化命令（如无则标注"无（IDE 格式化）"） | pnpm format / mvn spotless:apply | CLAUDE.md |
| `{{LINT_CMD}}` | Lint 检查命令（如无则标注"无"） | pnpm lint | CLAUDE.md、rules/testing.md |
...
| `{{E2E_FRAMEWORK}}` | E2E 测试框架及状态 | Playwright（已配置）/ 未配置 | CLAUDE.md |
| `{{PROJECT_DIR}}` | 项目根目录名 | ruoyi-admin | CLAUDE.md |
| `{{FRONTEND_FRAMEWORK}}` | 前端框架名称+版本（具体技术） | Vue3 + Element Plus | CLAUDE.md |
| `{{BACKEND_FRAMEWORK}}` | 后端框架名称+版本（具体技术） | SpringBoot 2.7 | CLAUDE.md |
| `{{DATABASE}}` | 数据库名称+版本（具体技术） | MySQL 8.0 | CLAUDE.md |
| `{{VERSION}}` | 版本号 | 2.7 / 3.x | CLAUDE.md |
| `{{DEPENDENCY_1}}` | 关键依赖1 | MyBatis-Plus | CLAUDE.md |
| `{{DEPENDENCY_2}}` | 关键依赖2 | Sa-Token | CLAUDE.md |
| `{{PURPOSE}}` | 依赖用途 | ORM 框架 / 权限认证 | CLAUDE.md |
| `{{PROJECT_ROOT_DIR}}` | 项目根目录名 | ruoyi-admin | CLAUDE.md |
| `{{DIR_TREE}}` | 目录树 | 实际目录结构文本 | CLAUDE.md |
| `{{ROUTE_FILE}}` | 路由文件 | src/router/index.js | CLAUDE.md |
| `{{HANDLER_DIR}}` | Handler目录 | src/controller/ | CLAUDE.md |
| `{{BIZ_LOGIC_FILE}}` | 业务逻辑文件 | src/service/ | CLAUDE.md |
| `{{CONFIG_DIR}}` | 配置目录 | src/main/resources/ | CLAUDE.md |
| `{{PAGE_DIR}}` | 页面目录 | src/views/ | CLAUDE.md |
| `{{ROUTE_CONFIG}}` | 路由配置 | src/router/ | CLAUDE.md |
| `{{KEY_FILE_1}}` | 关键文件1 | pom.xml | CLAUDE.md |
| `{{KEY_FILE_2}}` | 关键文件2 | application.yml | CLAUDE.md |
| `{{FREQUENCY}}` | 修改频率 | 低 / 中 / 高 | CLAUDE.md |
| `{{OPTION_A}}` | ADR选项A | SpringBoot | CLAUDE.md |
| `{{OPTION_B}}` | ADR选项B | Quarkus | CLAUDE.md |
| `{{DATE}}` | ADR日期 | 2026-04-27 | CLAUDE.md |
| `{{OLD_TECH}}` | 迁移旧技术 | Vue2 | CLAUDE.md |
| `{{NEW_TECH}}` | 迁移新技术 | Vue3 | CLAUDE.md |
| `{{FEATURE}}` | 禁用特性 | 某功能 | CLAUDE.md |
| `{{LINK}}` | 参考链接 | issue#123 | CLAUDE.md |
| `{{CORE_DIR}}` | 禁止修改的目录 | ruoyi-framework | CLAUDE.md |
| `{{CONFIG_FILE}}` | 禁止提交的配置 | application-prod.yml | CLAUDE.md |
| `{{MODULE}}` | 修改限制模块 | ruoyi-framework | CLAUDE.md |
| `{{OWNER}}` | 模块负责人 | 张三 | CLAUDE.md |
| `{{METHOD}}` | 通知方式 | Slack #dev频道 | CLAUDE.md |
| `{{ANTI_PATTERN}}` | 代码反模式 | var 声明 | CLAUDE.md |
| `{{TECH}}` | 废弃技术 | jQuery | CLAUDE.md |
| `{{ID}}` | ADR编号 | 001 | CLAUDE.md |
| `{{LOCATION}}` | 禁止写逻辑的位置 | Controller | CLAUDE.md |
| `{{CORRECT_LOCATION}}` | 正确位置 | Service | CLAUDE.md |
| `{{PITFALL_1}}` | 已知陷阱1 | 数据库字段类型不匹配 | CLAUDE.md |
| `{{PITFALL_2}}` | 已知陷阱2 | 前端路由与后端权限不同步 | CLAUDE.md |
| `{{CORRECT_PRACTICE}}` | 正确做法 | 使用统一的类型映射 | CLAUDE.md |
...
#### CLAUDE.md（补充：原 steer/foundation 内容已合并）

CLAUDE.md 现包含原 steer/foundation/ 三个文件的所有内容：
- 项目信息区：含版本号、构建命令、环境配置（原 CLAUDE.md）
- 数据库规范节：含字符集、方言差异（原 CLAUDE.md）
- 目录结构节：含修改导航、ADR（原 CLAUDE.md）
- 禁止行为清单：含修改限制、代码风格、已知陷阱（原 CLAUDE.md）

初始化时需替换 CLAUDE.md 中的以下占位符（含原 steer 文件的占位符）：
...
- `{{DB_TYPE}}` → 数据库
- `{{MIDDLEWARE}}` → 中间件
- `{{DIR_1}}`、`{{DIR_1_ROLE}}` → 核心目录及职责
- `{{DIR_2}}`、`{{DIR_2_ROLE}}` → 额外核心目录及职责
- `{{PROJECT_DIR}}` → 项目根目录名

**原 CLAUDE.md 占位符（现已合入 CLAUDE.md）**：
- `{{FRONTEND_FRAMEWORK}}`、`{{VERSION}}` → 前端框架+版本
- `{{BACKEND_FRAMEWORK}}`、`{{VERSION}}` → 后端框架+版本
- `{{DATABASE}}`、`{{VERSION}}` → 数据库+版本
- `{{PACKAGE_MANAGER}}` → 包管理器
- `{{BUILD_CMD}}` → 构建命令
- `{{E2E_TEST_CMD}}` → E2E 测试命令
- `{{E2E_FRAMEWORK}}` → E2E 测试框架及状态

**原 CLAUDE.md 占位符（现已合入 CLAUDE.md）**：
- `{{PROJECT_ROOT_DIR}}` → 项目根目录名
- `{{DIR_TREE}}` → 实际目录树
- `{{ROUTE_FILE}}`、`{{HANDLER_DIR}}` → 路由/Handler文件
- `{{OPTION_A}}`、`{{OPTION_B}}`、`{{REASON}}`、`{{DATE}}` → ADR

**原 CLAUDE.md 占位符（现已合入 CLAUDE.md）**：
- `{{CORE_DIR}}` → 禁止修改的目录
- `{{ANTI_PATTERN}}`、`{{TECH}}` → 代码风格强制项
- `{{PITFALL_1}}`、`{{CORRECT_PRACTICE}}` → 已知陷阱
...
.claude/skills/sds-buildfix.md
.claude/skills/sds-codereview.md
.claude/skills/sds-dev.md
.claude/skills/sds-plan.md
.claude/skills/sds-spec.md
.claude/hooks/post-edit-check.ps1
.claude/hooks/pre-bash-check.ps1
.claude/rules/frontend.md
.claude/rules/governance.md
...
.claude/rules/testing.md
.claude/settings.json
.claude/knowledge/troubleshooting.md
.claude/steer/domain/api.md
.claude/steer/domain/frontend.md
.claude/rules/governance.md
.claude/steer/domain/security.md
...
加上根目录的 `CLAUDE.md`（根据参考文档模板生成，不在 `find .claude` 输出中），共 32 个文件（31 个 .claude 文件 + 1 个 CLAUDE.md）。
...
| 技术栈正确 | CLAUDE.md | `{{FRONTEND_FRAMEWORK}}`、`{{BACKEND_FRAMEWORK}}` 已替换（原 CLAUDE.md 内容已合入） |
| 目录结构正确 | CLAUDE.md | `{{DIR_TREE}}` 已替换为实际目录树（原 CLAUDE.md 内容已合入） |
| 禁止目录正确 | CLAUDE.md | `{{CORE_DIR}}` 已替换（原 CLAUDE.md 内容已合入） |
| API glob 正确 | api.md | `fileMatchPattern` 已手动编辑为项目实际路径 |
| 构建命令正确 | CLAUDE.md | `{{BUILD_CMD}}` 已替换 |
