...
### 第一步：整理需求概要 + Plan 回溯映射

AI 根据对话历史和 plan.md（如有）整理需求概要，并向用户确认理解是否正确。
...
#### 1.2 Plan→Spec 字段映射（plan.md 存在时强制执行）

读取 `.claude/specs/{feature}/plan.md`，逐章节确认每个 Plan 内容在 Spec 中的承载点：

| Plan 章节 | 映射到 Spec 文件 | 映射方式 |
|-----------|-----------------|----------|
| 需求复述 | requirements.md §1 功能概述 | 原文复制为一句话描述和业务背景 |
| 影响范围 | requirements.md §3 影响范围 + design.md §11 文件产出清单 | 模块影响→需求文件，文件影响→设计文件 |
| 修改步骤 | design.md §8 数据流程图 + tasks.md §3 任务列表 | 步骤→流程图和任务拆分 |
| 风险评估 | design.md §9 风险评估 | 原文复制并补充技术实现层面风险 |
| 验收标准 | requirements.md §2 验收条件表 | 每个验收标准编号化为 AC |
| 预估工作量 | tasks.md §6 总计 | 工作量转化为任务粒度估算 |

映射结果向用户展示，确认无遗漏后继续生成 Spec 文件。

> **为什么**：Plan 的 6 个章节包含需求分析、影响范围、风险等关键信息。如果第一步只输出"一句话描述+验收条件"，Plan 中的修改步骤、风险评估、预估工作量等细节会丢失。映射表是信息完整传递的保证。
...
- 功能名称：{名称}
- 需求概要（来自第一步）
- Plan 文件：.claude/specs/{feature}/plan.md（必须 Read，逐章节对照，确保 Plan 内容完整映射到需求文档）

Plan→本文件映射规则：
- Plan「需求复述」→ §1 功能概述（原文复制为一句话描述和业务背景）
- Plan「影响范围」→ §3 影响范围（模块影响）
- Plan「验收标准」→ §2 验收条件表（编号化为 AC）
- Plan「风险评估」中的业务风险 → §7 边界情况（标注来源：plan.md）
...
- 功能名称：{名称}
- 验收条件：AC-001...AC-00N
- Plan 文件：.claude/specs/{feature}/plan.md（必须 Read，逐章节对照，确保 Plan 内容完整映射到设计文档）
- 需求文档：.claude/specs/{feature}/requirements.md（必须 Read，确保设计覆盖所有 AC 和 BR）

Plan→本文件映射规则：
- Plan「影响范围」中的文件影响 → §11 文件产出清单
- Plan「修改步骤」→ §8 数据流程图（步骤转化为流程节点）
- Plan「风险评估」→ §9 风险评估（原文复制并补充技术实现层面风险）
- Plan「预估工作量」→ §6 性能约束中的数据量预估部分

输出文件：.claude/specs/{feature}/design.md

必须包含以下章节（按模板格式）：
1. 技术方案概述（一句话描述 + 方案选择原因 + 需求追溯矩阵）
2. 数据库设计（新增表/修改表/回滚脚本，**每个新增表必须输出完整 DDL（CREATE TABLE + COMMENT + CREATE INDEX），不得仅提供字段定义表格**，DDL 语法必须使用 PostgreSQL，标注 `-- DB_TYPE: PostgreSQL`）
...
- 功能名称：{名称}
- 验收条件：AC-001...AC-00N
- 设计文档：.claude/specs/{feature}/design.md（必须 Read）
- 需求文档：.claude/specs/{feature}/requirements.md（必须 Read）
- Plan 文件：.claude/specs/{feature}/plan.md（必须 Read）
  - Plan「验收标准」→ 确认测试用例覆盖所有验收条件
  - Plan「风险评估」→ 确认高风险项有对应边界测试
  - Plan「影响范围」→ 确认影响模块有对应集成测试
...
- 功能名称：{名称}
- 设计文档：.claude/specs/{feature}/design.md（必须 Read）
- 测试用例：.claude/specs/{feature}/test-cases.md（必须 Read）
- 需求文档：.claude/specs/{feature}/requirements.md（必须 Read）
- Plan 文件：.claude/specs/{feature}/plan.md（必须 Read）
  - Plan「修改步骤」→ 确认任务清单覆盖所有修改步骤
  - Plan「预估工作量」→ §6 总计中的工作量估算（对照 Plan 的后端/前端/数据库/测试数量）
  - Plan「影响范围」中的文件列表 → 确认任务清单的文件产出与 Plan 一致
...
- [ ] design.md：数据库设计章节每个新增表有完整 DDL（CREATE TABLE + COMMENT + CREATE INDEX），不得仅提供字段定义表格
- [ ] design.md：DDL 语法为 PostgreSQL，标注 `-- DB_TYPE: PostgreSQL`，使用双引号标识符
...
### Plan 回溯验证（plan.md 存在时强制执行）

逐一对照 `.claude/specs/{feature}/plan.md` 的 6 个章节，确认每个章节内容都有对应的 Spec 承载点：

- [ ] Plan「需求复述」→ requirements.md §1 功能概述 已包含
- [ ] Plan「影响范围」→ requirements.md §3 + design.md §11 已包含
- [ ] Plan「修改步骤」→ design.md §8 数据流程图 + tasks.md §3 任务列表 已包含
- [ ] Plan「风险评估」→ design.md §9 风险评估 已包含
- [ ] Plan「验收标准」→ requirements.md §2 验收条件表 已编号化
- [ ] Plan「预估工作量」→ tasks.md §6 总计 已量化

> 如果某个 Plan 章节在 Spec 中找不到对应承载点，必须在 Spec 文件中补充，并标注来源 `plan.md §X`。
...
9. **Plan 回溯验证** — plan.md 存在时，必须逐章节对照确认 Plan 内容完整映射到 Spec，缺失项须在 Spec 中补充并标注来源 `plan.md §X`
10. **Plan 持久化依赖** — sds-spec 的 Plan 回溯依赖 sds-plan 已保存 plan.md 文件；如 plan.md 不存在（用户直接调用 `/sds-spec`），则跳过回溯步骤，仅依据对话历史和用户描述生成